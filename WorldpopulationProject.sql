...
-- Find the country, capital city and continent
...
-- Find the the highest and lowest population in 2022
...
-- Find the country whose population equals the highest population in 2022
SELECT country, 2022_population AS population
FROM world_population1
...
-- Find the percentile rank of the 2022 population by country while partitioned by continent
SELECT
    country,
    2022_population,
    continent,
...
SELECT MAX(growth_rate) AS max_pop_growth,
	   MIN(growth_rate) min_pop_growth 
  FROM world_population1;
